...
Les fichiers essentiels sont `index.html`, `styles.css`, `app.js`, `manifest.webmanifest`, `service-worker.js`, les icônes et le nouveau fichier `runtime-config.js`. Après la publication GitHub Pages, videz le cache du navigateur ou rechargez la page avec un rechargement forcé afin que le nouveau service worker remplace l’ancien.
